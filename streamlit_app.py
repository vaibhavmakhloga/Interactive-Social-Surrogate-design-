...
def main():
    ...
    if not st.session_state.started:
        if display_instructions():
            ...
            # Generate background story immediately after clicking start
            with st.spinner("🤖 Generating background story..."):
                ...
                agent_b = st.session_state.agents[1]
                ...
                try:
                    ...
                    st.error(f"Error generating background story: {str(e)}")
            st.rerun()
        return

    # Only show the rest of the interface if started
    if st.session_state.started:
        # Remove the background story generation from here since it's now handled above
        # Create two columns
