...
def main():
    ...
    with col2:
        ...
        if show_challenges and st.session_state.story_manager.challenge_history:
            for i, challenge in enumerate(st.session_state.story_manager.challenge_history):
                # Get the raw challenge text from Agent C
                ...
                if i < len(st.session_state.story_manager.challenge_history) - 1:
                    st.markdown("<div style='margin: 10px 0; text-align: center; color: #95a5a6;'>↓</div>", 
                              unsafe_allow_html=True)
