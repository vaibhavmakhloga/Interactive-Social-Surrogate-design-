...
def main():
    ...
    with st.sidebar:
        ...
        <div style='background-color: #1a1a1a; padding: 20px; border-radius: 10px;'>
            ...
            <ul style='color: #ffffff; list-style-type: none; padding-left: 0;'>
                <li style='margin-bottom: 10px;'>1. Be specific about your features and reasoning</li>
                <li style='margin-bottom: 10px;'>2. Build on previous stories</li>
                <li style='margin-bottom: 10px;'>3. Focus on one feature at a time</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.session_state.get('ended', False):
        if st.session_state.get('save_status', False):
            st.success("✨ All session data successfully saved to database!")
        else:
        ...
        # Success message
        st.markdown("<h1 style='color: #1f77b4;'>Session Complete</h1>", unsafe_allow_html=True)
        
        # Get all features from the session
        if st.session_state.story_manager.user_prompts:
            st.markdown("<h2 class='rank-header'>Rank Your Features</h2>", unsafe_allow_html=True)
            st.markdown("Please rank your features from most important (1) to least important:")
            ...
            # Create ranking section for each feature
            for i, feature in enumerate(features, 1):
                st.markdown(f"""
                    <div class='feature-card'>
                        <strong>Feature {i}:</strong> {feature}
                    </div>
                """, unsafe_allow_html=True)
                
                rankings[feature] = st.number_input(
                    f"Rank (1-{total_features})",
                    min_value=1,
            ...
            if st.button("💾 Save Rankings", type="primary"):
                # Convert rankings to a structured format
                final_rankings = [
                ...
                if st.session_state.story_manager.save_feature_rankings(final_rankings):
                    st.success("Rankings saved! Thank you for your participation! 🎉")
        ...
        with col1:
            ...
            with st.expander("🤓 Stats for Nerds"):
                ...
                with tab2:
                    ...
                    # Convert chapter numbers to actual dimension names
                    covered_dimensions = [st.session_state.story_manager.get_actual_dimension(ch) for ch in progress['covered']]
                    remaining_dimensions = [st.session_state.story_manager.get_actual_dimension(ch) for ch in progress['remaining']]
                    
                    st.markdown(f"""
                    - **Covered**: {', '.join(covered_dimensions) if covered_dimensions else 'None'}
                    - **Remaining**: {', '.join(remaining_dimensions)}
